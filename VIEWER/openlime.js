	...
	class UIBasic {
		...
		constructor(viewer, options) {
			...
			for (let [id, layer] of Object.entries(this.viewer.canvas.layers)) {
				...
				if (layer.annotations) {
					// 如果有注释，添加注释条目
					this.menu.push(layer.annotationsEntry());
				}
			}
		...
		init() {
			(async () => {
				...
				document.addEventListener('click', (e) => {
					// 检查菜单是否打开
					if (this.layerMenu && this.layerMenu.classList.contains('open')) {
						// 检查点击的元素是否在菜单内部或者是菜单按钮
						const isClickInsideMenu = this.layerMenu.contains(e.target);
						const isClickOnMenuButton = this.actions.layers.element && this.actions.layers.element.contains(e.target);
		...
		createEntry(entry) {
			...
			if ('title' in entry) {
				...
				html += `<button ${id} ${group} ${layer} ${mode} ${tooltip} class="openlime-entry ${classes}">${entry.button}</button>`;
